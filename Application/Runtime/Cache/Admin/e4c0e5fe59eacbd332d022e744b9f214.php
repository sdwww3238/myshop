...
<div style="font-size: 13px;margin: 10px 5px">
    <form action="<?php echo U('update');?>" method="post" enctype="multipart/form-data">
        ...
        <table border="1" width="100%" class="table_a" id="general-tab-tb">
            ...
            <tr>
                ...
                <td>
                    <select name='cat_id' id="cat_id_0">
                        <?php if(is_array($catinfo)): foreach($catinfo as $key=>$v): if(($info['cat_id']) == $v["cat['id']"]): ?><option value='<?php echo ($v["cat_id"]); ?>' selected="selected">
                                    <?php else: ?>
            ...
            <tr>
                <td>扩展分类</td>
                <td><input type="button" value="添加" onclick="add_sel(this)"/>
            ...
            <td>
                ...
                <?php if(!empty($info["goods_big_logo"])): ?><img src='<?php echo ($info["goods_big_logo"]); ?>' width='200' height='200' alt=''/>
        ...
        >
            <tr>
                <td><label>商品详细描述：</label></td>
                <td>
                    <script name='goods_introduce' type="text/plain" id="myEditor" style="width:666px;height:240px;"><?php echo ($info["goods_introduce"]); ?>
                    </script>
                </td>
            </tr>
            <script type='text/javascript'>
